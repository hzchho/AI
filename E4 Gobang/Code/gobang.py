...
class Gobang(game.Game):
    def __init__(self, title, size, fps=15, chess_file=None,save_file=None): 
    ...
    def AI_player(self):
        pygame.display.set_caption("五子棋"+self.chess_file+"---- AI思考中")
        time1=time.time()
        x, y, alpha=Search(self.board, EMPTY, BLACK, WHITE, self.black)
        self.count+=1
        time2=time.time()
        pygame.display.set_caption("五子棋"+self.chess_file)
        self.click(y*SIDE, x*SIDE)
        print(x, y, alpha)
        print("Used time:",time2-time1,'s')
        self.file.write("No"+str(self.count)+":\n")
        self.file.write("Located:"+str(x)+","+str(y)+"\nScore:"+str(alpha)+'\n')
        self.file.write("Used time:"+str(time2-time1)+'s\n\n')
...
if __name__ == '__main__':
    ...
    gobang = Gobang("五子棋", (SCREEN_WIDTH, SCREEN_HEIGHT), chess_file="example/1.txt",save_file="result/1.txt")
    #gobang = Gobang("五子棋", (SCREEN_WIDTH, SCREEN_HEIGHT), chess_file=None)
    gobang.run()
